...
static llvm::cl::OptionCategory ParserCategory("Tooling matcher");
...
CDumpMatcher::CDumpMatcher() 
{
}

bool
CDumpMatcher::CDumpMatcher::init(int _argc, const char *_argv[])
{
	m_argc = _argc;
	m_argv = _argv;
}
...
bool 
CDumpMatcher::parse(const std::string &_file, const std::string &_class, CDumpAST &_ast)
{
	argc = 1;
	char *argv[] = {_file};
...
bool
CDumpMatcher::match(const std::string &_class, const std::string &_file, MatchFinder::MatchCallback &_cb)
{
	argc = 1;
	char *argv[] = {_file};
...
//	RecordDumper recoredHandler;
	...
	finder.addMatcher(cb(hasName(_class)).bind("stuff"), &recoredHandler);
